...
function syntheticDataset() {
  ...
  for (let i = 0; i < 25; i++) {
    data.push({
      features: [2 + rng() * 1.8, 2 + rng() * 1.8],
      label: 0
    });
  }
  for (let i = 0; i < 25; i++) {
    data.push({
      features: [6 + rng() * 1.8, 6 + rng() * 1.8],
      label: 1
    });
  }
  ...
  return {
    ...
    description: 'Datos sintéticos 2D linealmente separables con ruido'
  };
}
...
function irisDataset() {
  ...
  return {
    samples: data,
    featureNames: ['Largo pétalo (cm)', 'Ancho pétalo (cm)'],
    classNames: ['Setosa', 'Versicolor'],
    description: 'Iris: clasificación Setosa vs Versicolor por medidas del pétalo'
  };
}
